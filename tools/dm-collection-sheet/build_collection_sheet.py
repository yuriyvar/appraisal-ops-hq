...
CATALOG = [
 ...
 # DM names below VERIFIED 2026-07-02 against the 113-file .dma corpus registry
 # (Past Reports/_analysis/_dma-decode/corpus_field_names.json). In these domains DM
 # registers ONLY: ContractDate(Known/Unknown) · HasConcessions/ConcessionAmount/
 # FinancialAssistanceUnknownAmount · FinanceTypes · ChangeInLandUse_* · MarketAreaVacant_*.
 # The URAR Neighborhood-trend fields are NOT DM-importable — empty dm list = enter
 # DIRECTLY in ACI (quirk DMA-003). Collect them here regardless; the form needs them.
 ("Contract", [
  ("Contract price", "CONTRACT_PRICE", [],
   [], "MLS/PUB", "purchase", True,
   "Purchase only - N/A on a refinance. Not in DM registry - enter in ACI."),
  ("Date of contract", "CONTRACT_DATE",
   ["ContractDate","ContractDateKnown","ContractDateUnknown"],
   [], "MLS", "purchase", True, ""),
  ("Seller = owner of public record?", "SELLER_OWNER", [],
   [], "PUB", "purchase", True, "Yes / No. Not in DM registry - enter in ACI."),
  ("Seller concessions ($ + paid by)", "CONCESSIONS",
   ["HasConcessions","ConcessionAmount","FinancialAssistanceUnknownAmount"],
   [], "MLS", "purchase", True,
   "Amount + who paid; concessions adjust the comp prices."),
  ("Financing type", "FINANCING", ["FinanceTypes"],
   [], "MLS/order", "purchase", True, "Conv / FHA / VA / Cash / Other."),
 ]),
 ("Neighborhood", [
  # ACI-direct section: DM has no registry names for the trend fields (DMA-003).
  ("Location", "NBHD_LOCATION", [],
   [], "APEX/INSP", "all", True, "Urban / Suburban / Rural."),
  ("Built-up", "NBHD_BUILTUP", [],
  ...
  ("Neighborhood boundaries", "NBHD_BOUNDARIES", [],
   [], "INSP", "all", True, "N / S / E / W bounding roads or features."),
  ("Present land use %", "NBHD_LANDUSE", [],
   [], "APEX/INSP", "all", True,
   "1-unit / 2-4 / multi / commercial / other = 100%. DM only registers "
   "ChangeInLandUse_(InProcess/Likely/NotLikely) + MarketAreaVacant_0To5Percent (gPAR)."),
  ...
  ("Market conditions (remarks)", "NBHD_MARKET_COND", [],
