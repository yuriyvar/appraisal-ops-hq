...
CATALOG = [
 ...
 # NOTE: Neighborhood + Contract DM field names below are UAD-convention BEST-EFFORT.
 # Verify against the .dma registry (tools/dma-decoder) before trusting the MD ref's DM column.
 # (Names show only in the MD reference, not the HTML worksheet the appraiser pastes.)
 ("Contract", [
  ("Contract price", "CONTRACT_PRICE", ["ContractPrice","SalesContractAmount"],
   [], "MLS/PUB", "purchase", True, "Purchase only - N/A on a refinance."),
  ("Date of contract", "CONTRACT_DATE", ["ContractDate"],
   [], "MLS", "purchase", True, ""),
  ("Seller = owner of public record?", "SELLER_OWNER", ["SellerIsOwnerOfRecord"],
   [], "PUB", "purchase", True, "Yes / No."),
  ("Seller concessions ($ + paid by)", "CONCESSIONS",
   ["SellerConcessions","ConcessionAmount"], [], "MLS", "purchase", True,
   "Amount + who paid; concessions adjust the comp prices."),
  ("Financing type", "FINANCING", ["FinancingType","LoanType"],
   [], "MLS/order", "purchase", True, "Conv / FHA / VA / Cash / Other."),
 ]),
 ("Neighborhood", [
  ("Location", "NBHD_LOCATION", ["LocationUrban","LocationSuburban","LocationRural"],
   [], "APEX/INSP", "all", True, "Urban / Suburban / Rural."),
  ("Built-up", "NBHD_BUILTUP", ["BuiltUpOver75","BuiltUp25To75","BuiltUpUnder25"],
   [], "INSP", "all", True, "Over 75% / 25-75% / Under 25%."),
  ("Growth", "NBHD_GROWTH", ["GrowthRapid","GrowthStable","GrowthSlow"],
   [], "INSP", "all", True, "Rapid / Stable / Slow."),
  ("Property values trend", "NBHD_VALUES",
   ["PropertyValuesIncreasing","PropertyValuesStable","PropertyValuesDeclining"],
   [], "MLS/INSP", "all", True, "Increasing / Stable / Declining."),
  ("Demand / supply", "NBHD_DEMAND",
   ["DemandSupplyShortage","DemandSupplyInBalance","DemandSupplyOverSupply"],
   [], "MLS/INSP", "all", True, "Shortage / In balance / Over supply."),
  ("Marketing time", "NBHD_MKTG_TIME",
   ["MarketingTimeUnder3","MarketingTime3To6","MarketingTimeOver6"],
   [], "MLS", "all", True, "Under 3mo / 3-6mo / Over 6mo."),
  ("Neighborhood boundaries", "NBHD_BOUNDARIES", ["NeighborhoodBoundaries"],
   [], "INSP", "all", True, "N / S / E / W bounding roads or features."),
  ("Present land use %", "NBHD_LANDUSE",
   ["PresentLandUseOneUnit","PresentLandUse2To4","PresentLandUseMultiFamily",
    "PresentLandUseCommercial","PresentLandUseOther"],
   [], "APEX/INSP", "all", True, "1-unit / 2-4 / multi / commercial / other = 100%."),
  ("One-unit housing price ($000 low/high/pred)", "NBHD_PRICE",
   ["OneUnitHousingPriceLow","OneUnitHousingPriceHigh","OneUnitHousingPricePredominant"],
   [], "MLS", "all", True, "Low / High / Predominant sale price."),
  ("One-unit housing age (yrs low/high/pred)", "NBHD_AGE",
   ["OneUnitHousingAgeLow","OneUnitHousingAgeHigh","OneUnitHousingAgePredominant"],
   [], "MLS/APEX", "all", True, "Low / High / Predominant age."),
  ("Market conditions (remarks)", "NBHD_MARKET_COND", ["NeighborhoodMarketConditions"],
   [], "MLS", "all", True, "TBD until comps assembled (trend + absorption)."),
 ]),
 ("Site", [
