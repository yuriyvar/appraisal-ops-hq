...
A dedicated, **bidirectional memo channel between the two lanes** — Cowork/COWORK_AGENT ⇄ Claude Code.
Lives in the repo because both lanes can reach it (Cowork is locked out of `.claude/`).

> **`COWORK_AGENT`** in these docs = the current Cowork agent persona (presently **Ton**, formerly Bob).
> Cowork agents get swapped periodically, so the docs refer to the agent **symbolically** as `COWORK_AGENT`
> rather than by name. On rename, change the real name in just **two** spots: this line, and the
> `COWORK_AGENT` constant in `tools/dm-collection-sheet/build_collection_sheet.py` (the only place it is
> rendered into a worksheet). Everything else stays symbolic.
...
|---|---|---|
| `INBOX-for-Code.md` | **Cowork/COWORK_AGENT** | Claude Code |
| `INBOX-for-Cowork.md` | **Claude Code** | Cowork/COWORK_AGENT |
...
- Cowork/COWORK_AGENT → reads `INBOX-for-Cowork.md`
- Claude Code → reads `INBOX-for-Code.md`
(Wired into the bootstrap: START-HERE for Code; the Cowork prep flow / START-HERE lookup for COWORK_AGENT.)
